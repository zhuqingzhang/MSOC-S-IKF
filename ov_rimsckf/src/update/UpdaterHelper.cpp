...
void UpdaterHelper::get_feature_jacobian_full(State* state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
    ...
    for (auto const& pair : feature.timestamps) {
        ...
        for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
            ...
            uv_m << (double)feature.uvs[pair.first].at(m)(0), (double)feature.uvs[pair.first].at(m)(1);
            res.block(2*c,0,2,1) = uv_m - uv_dist;
            // cout<<"cam_d: "<<cam_d.transpose()<<endl;
            // cout<<"uv_norm: "<<uv_norm.transpose()<<endl;
            // cout<<"uv_m: "<<uv_m.transpose()<<" uv_dist: "<<uv_dist<<endl;
...
void UpdaterHelper::get_feature_jacobian_kf(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_n_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    total_meas=feature.keyframe_matched_obs.size();
    ...
    // If we are using an anchored representation, make sure that the anchor is added
    ...
    res = Eigen::VectorXd::Zero(2*total_meas);
    H_f = Eigen::MatrixXd::Zero(2*total_meas,jacobsize);
    H_n_x = Eigen::MatrixXd::Zero(2*total_meas,total_hx);
    ...
    for (auto const& pair : feature.keyframe_matched_obs)
    {
        for(auto const& match : pair.second )
        {
            ...
            //transform the feature to kf frame;
            Eigen::Vector3d p_FinW = R_GtoW * p_FinG +p_GinW;
            Eigen::Vector3d p_FinKF= R_KFtoW.transpose()*(p_FinW-p_KFinW);

            Eigen::Matrix<double,2,1> uv_norm;
            uv_norm<<p_FinKF(0)/p_FinKF(2),p_FinKF(1)/p_FinKF(2);
            ...
            if(is_fisheye) {
                ...
                double x1 = uv_norm(0)*cdist;
                double y1 = uv_norm(1)*cdist;
                uv_dist(0) = kf_d(0)*x1 + kf_d(2);
                uv_dist(1) = kf_d(1)*y1 + kf_d(3);
                ...
                double r_2 = r*r;
                double r_4 = r_2*r_2;
                double x1 = uv_norm(0)*(1+kf_d(4)*r_2+kf_d(5)*r_4)+2*kf_d(6)*uv_norm(0)*uv_norm(1)+kf_d(7)*(r_2+2*uv_norm(0)*uv_norm(0));
                double y1 = uv_norm(1)*(1+kf_d(4)*r_2+kf_d(5)*r_4)+kf_d(6)*(r_2+2*uv_norm(1)*uv_norm(1))+2*kf_d(7)*uv_norm(0)*uv_norm(1);
                uv_dist(0) = kf_d(0)*x1 + kf_d(2);
                uv_dist(1) = kf_d(1)*y1 + kf_d(3);
            ...
            // Our residual
            Eigen::Matrix<double,2,1> uv_m;
            uv_m << (double)keyframe->point_2d_uv[kf_feature_id].x, (double)keyframe->point_2d_uv[kf_feature_id].y;
            cout<<"measure feature: "<<uv_m.transpose()<<endl;
            cout<<"predict feature: "<<uv_dist.transpose()<<endl;
            res.block(2*c,0,2,1) = uv_m - uv_dist;
            ...
            if(state->_options.do_fej) {
                //transform the feature to kf frame;
                p_FinW = R_GtoW * p_FinG_fej +p_GinW;
                p_FinKF= R_KFtoW.transpose()*(p_FinW-p_KFinW);
                p_FinG=p_FinG_fej;
                //uv_norm << p_FinCi(0)/p_FinCi(2),p_FinCi(1)/p_FinCi(2);
                //cam_d = state->get_intrinsics_CAM(pair.first)->fej();
            }
            ...
            Eigen::Matrix<double,2,2> dz_dzn = Eigen::Matrix<double,2,2>::Zero();
            Eigen::Matrix<double,2,8> dz_dzeta = Eigen::Matrix<double,2,8>::Zero();
            UpdaterHelper::get_feature_jacobian_intrinsics(state, uv_norm, is_fisheye, kf_d, dz_dzn, dz_dzeta);

            //Compute Jacobian of transforming feature in kf frame into nomalized uv frame;
            //dhp_dpfc
            Eigen::Matrix<double,2,3> dzn_dpfc = Eigen::Matrix<double,2,3>::Zero();
            dzn_dpfc << 1/p_FinKF(2),0,-p_FinKF(0)/(p_FinKF(2)*p_FinKF(2)),
                    0, 1/p_FinKF(2),-p_FinKF(1)/(p_FinKF(2)*p_FinKF(2));

            // Derivative of p_FinKF with respect to p_FinW
            //dht_dpfw
            Eigen::Matrix<double,3,3> dpfc_dpfw = R_KFtoW.transpose();

            // Derivative of p_FinKF in respect to KF state(nuisance state)
            //dht_dxn
            Eigen::Matrix<double,3,6> dpfc_dxn = Eigen::Matrix<double,3,6>::Zero();
            dpfc_dxn.block(0,0,3,3).noalias() = -R_KFtoW.transpose()*skew_x(p_FinW-p_KFinW);
            dpfc_dxn.block(0,3,3,3) = -dpfc_dpfw;

            //Derivative of P_FinW with respect to p_FinG
            //dhs_dpfg
            Eigen::Matrix<double,3,3> dpfw_dpfg = R_GtoW;

            // Precompute some matrices
            Eigen::Matrix<double,2,3> dz_dpfc = dz_dzn*dzn_dpfc;
            Eigen::Matrix<double,2,3> dz_dpfw = dz_dpfc*dpfc_dpfw;
            Eigen::Matrix<double,2,3> dz_dpfg = dz_dpfw*dpfw_dpfg;

            //Derivative of P_FinW with respect to transformation between map and vio
            //dhs_dx
            Eigen::Matrix<double,3,6> dpfw_dx =Eigen::Matrix<double,3,6>::Zero();
            dpfw_dx.block(0,0,3,3)=skew_x(R_GtoW*p_FinG);
            dpfw_dx.block(0,3,3,3)=MatrixXd::Identity(3,3);

            // CHAINRULE: get the total feature Jacobian
            H_f.block(2*c,0,2,H_f.cols()).noalias() = dz_dpfg*dpfg_dlambda;

            H_n.block(2*c, map_hn[kf_w],2,kf_w->size()).noalias() = dz_dpfc*dpfc_dxn;

            // CHAINRULE: loop through all extra states and add their
            // NOTE: we add the Jacobian here as we might be in the anchoring pose for this measurement
            for(size_t i=0; i<dpfg_dx_order.size(); i++) {
                H_n_x.block(2*c,map_hx[dpfg_dx_order.at(i)],2,dpfg_dx_order.at(i)->size()).noalias() += dz_dpfg*dpfg_dx.at(i);
            }
            H_n_x.block(2*c,map_hx[tranform],2,tranform->size()).noalias() = dz_dpfw*dpfw_dx;


            //test Jacobian
            Vector2d z_fdist,z_fdist_turb;
            compute_meas(p_FinG, p_GinW, R_GtoW,p_KFinW, R_KFtoW, kf_d, z_fdist);

            Vector3d turb(0.0001, -0.003, 0.003);
            //turb on p_FinG:
            compute_meas(p_FinG+turb, p_GinW, R_GtoW,p_KFinW, R_KFtoW, kf_d, z_fdist_turb);
            cout<<"turb on p_FinG:"<<endl;
            cout<<"diff: "<<(z_fdist_turb-z_fdist).transpose()<<endl;
            cout<<"diff jacob: "<<(dz_dpfg*turb).transpose()<<endl;
            ...
            compute_meas(p_FinG, p_GinW, R_GtoW_turb,p_KFinW, R_KFtoW, kf_d, z_fdist_turb);
            cout<<"turb on R_GtoW:"<<endl;
            cout<<"diff: "<<(z_fdist_turb-z_fdist).transpose()<<endl;
            cout<<"diff jacob: "<<(dz_dpfw*(dpfw_dx.block(0,0,3,3))*turb).transpose()<<endl;

            //turb on p_GtoW:
            compute_meas(p_FinG, p_GinW+turb, R_GtoW,p_KFinW, R_KFtoW, kf_d, z_fdist_turb);
            cout<<"turb on p_GinW:"<<endl;
            cout<<"diff: "<<(z_fdist_turb-z_fdist).transpose()<<endl;
            cout<<"diff jacob: "<<(dz_dpfw*(dpfw_dx.block(0,3,3,3))*turb).transpose()<<endl;

            //turb on R_KFtoW:
            q=quat_multiply(dq, kf_w->quat());
            q_JPL->set_value(q);
            Matrix3d R_KFtoW_turb=q_JPL->Rot();
            compute_meas(p_FinG, p_GinW, R_GtoW,p_KFinW, R_KFtoW_turb, kf_d, z_fdist_turb);
            cout<<"turb on R_KFtoW:"<<endl;
            cout<<"diff: "<<(z_fdist_turb-z_fdist).transpose()<<endl;
            cout<<"diff jacob: "<<(dz_dpfc*(dpfc_dxn.block(0,0,3,3))*turb).transpose()<<endl;

            //turb on p_KFinW:
            compute_meas(p_FinG, p_GinW, R_GtoW,p_KFinW+turb, R_KFtoW, kf_d, z_fdist_turb);
            cout<<"turb on R_KFtoW:"<<endl;
            cout<<"diff: "<<(z_fdist_turb-z_fdist).transpose()<<endl;
            cout<<"diff jacob: "<<(dz_dpfc*(dpfc_dxn.block(0,3,3,3))*turb).transpose()<<endl;
...
void UpdaterHelper::get_feature_jacobian_kf2(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
    ...
    res = Eigen::VectorXd::Zero(4*total_meas);
    H_f = Eigen::MatrixXd::Zero(4*total_meas,jacobsize);
    H_x = Eigen::MatrixXd::Zero(4*total_meas,total_hx);
    H_n = Eigen::MatrixXd::Zero(4*total_meas,total_hn);
    ...
    for (auto const& pair : feature.keyframe_matched_obs)
    {
        for(auto const& match : pair.second )
        {
            ...
            cv::Point3f p_fink=keyframe->point_3d[kf_feature_id];
            Vector3d p_FinKF(double(p_fink.x),double(p_fink.y),double(p_fink.z));

            cv::Point2f uv_ob=keyframe->matched_point_2d_uv[kf_feature_id];
            double x=uv_ob.x;
            double y=uv_ob.y;
            Vector2d uv_OB(x,y);
            ...
            dpfw_dxn.block(0,0,3,3)=skew_x(R_KFtoW*p_FinKF);
            ...
            //dhw_dpfw
            Eigen::Matrix<double,3,3> dpfg_dpfw= R_GtoW.transpose();
            //dhw_dxtrans
            Eigen::Matrix<double,3,6> dpfg_dxtrans=Eigen::Matrix<double,3,6>::Zero();
            dpfg_dxtrans.block(0,0,3,3)=-R_GtoW.transpose()*skew_x(p_FinW-p_GinW);
            dpfg_dxtrans.block(0,3,3,3)=-R_GtoW.transpose();
            ...
            //dht_dpfg
            Eigen::Matrix3d dpfc_dpfg = R_ItoC*R_GtoI;
            //dht_dxcur
            Eigen::Matrix<double,3,6> dpfc_dxcur=Eigen::Matrix<double,3,6>::Zero();
            dpfc_dxcur.block(0,0,3,3)=R_ItoC*skew_x(R_GtoI*(p_FinG-p_IinG));
            dpfc_dxcur.block(0,3,3,3)=-R_ItoC*R_GtoI;
            
            Eigen::Matrix<double,3,6> dpfc_dxe=Eigen::Matrix<double,3,6>::Zero();;
            if(state->_options.do_calib_camera_pose)
            {

               dpfc_dxe.block(0,0,3,3)=skew_x(R_ItoC*R_GtoI*(p_FinG-p_IinG));
            ...
            Eigen::Matrix<double,2,2> dz_dzn = Eigen::Matrix<double,2,2>::Zero();
            Eigen::Matrix<double,2,8> dz_dzeta = Eigen::Matrix<double,2,8>::Zero();
            UpdaterHelper::get_feature_jacobian_intrinsics(state, uv_norm, state->_cam_intrinsics_model.at(0), cam_d, dz_dzn, dz_dzeta);

            //Compute Jacobian of transforming feature in kf frame into nomalized uv frame;
            //dhp_dpfc
            Eigen::Matrix<double,2,3> dzn_dpfc = Eigen::Matrix<double,2,3>::Zero();
            dzn_dpfc << 1/p_FinC(2),0,-p_FinC(0)/(p_FinC(2)*p_FinC(2)),
                    0, 1/p_FinC(2),-p_FinC(1)/(p_FinC(2)*p_FinC(2));
...
bool UpdaterHelper::get_feature_jacobian_kf3(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        for(int i=0;i<feature.timestamps[0].size();i++)
        {
            ...
            Eigen::Matrix<double,3,6> dpfc_dxe=Eigen::Matrix<double,3,6>::Zero();
            Eigen::Matrix<double,3,6> dpfc_dkfanchor = Eigen::Matrix<double,3,6>::Zero();
            dpfc_dkfanchor.block(0,0,3,3) =  - R_GtoC_linp * skew_x( p_FinW);
            // if(state->_options.do_calib_camera_pose)
            // {
                   //jacobian is of R_CtoI p_CinI
            //     dpfc_dxe.block(0,0,3,3)= R_GtoC * (skew_x(p_WinG + p_IinG + R_WtoG * p_FinW)* R_GtoI.transpose())
            //     dpfc_dxe.block(0,3,3,3)= -R_GtoC * R_GtoI.transpose();
            // }
            ...
            if(!state->_options.ptmeas)
            {
              ...
              H_n.block(2*c,map_hn[anchor_kf_w],2,anchor_kf_w->size()).noalias()=dz_dpfc*dpfc_dkfanchor;
...
//           sleep(1);
           ...
           if(is_fisheye) {
            ...
            dpfk_dxnanchor.block(0,0,3,3) = -R_KFtoW.transpose() * skew_x(p_FinW);
            dpfk_dxnanchor.block(0,3,3,3) = Matrix<double,3,3>::Zero();
            if(state->_options.ptmeas)
            {
...
bool UpdaterHelper::get_feature_jacobian_kf4(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
...
bool UpdaterHelper::get_feature_jacobian_kf5(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        Eigen::Matrix3d dpfk_dpfw = R_AnchorKFtoW.transpose();
        Eigen::Matrix<double,3,6> dpfk_dxnanchor = Eigen::Matrix<double,3,6>::Zero();
        dpfk_dxnanchor.block(0,0,3,3) = R_AnchorKFtoW.transpose() * skew_x(p_FinW-p_AnchorKFinW); 
        dpfk_dxnanchor.block(0,3,3,3) = -R_AnchorKFtoW.transpose();
...
//           sleep(1);
           ...
           if(is_fisheye) {
            ...
            dpfk_dpfw = R_KFtoW.transpose();
            Matrix<double,3,6> dpfk_dxkf = Matrix<double,3,6>::Zero();
            dpfk_dxkf.block(0,0,3,3) = R_KFtoW.transpose() * skew_x(p_FinW-p_KFinW);
            dpfk_dxkf.block(0,3,3,3) = -R_KFtoW.transpose();
...
bool UpdaterHelper::get_feature_jacobian_oc2(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
   ...
   //! dh_dpI=-dh_dpLG, dh_dRI=-dh_dR_LG, dh_dpF=dh_dpLG*R_LG0
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        MatrixXd A = MatrixXd::Zero(2,9);
        //fill in u
        u.block(0,0,3,3) = -R_WtoG_fej;
        u.block(6,0,3,3) = Eigen::Matrix3d::Identity();
        u.block(3,3,3,3) = Eigen::Matrix3d::Identity();
        u.block(6,3,3,3) = skew_x(p_FinW)*R_WtoG_fej.transpose();
        ...
        for(int i=0;i<feature.timestamps[0].size();i++)
        {
            ...
            //now we use newset Jacobian to compute the optimized Jacobian that satisfy the oc
            A.block(0,0,2,3) = dz_dpfc * dpfc_dxtrans.block(0,3,3,3); //p_LG
            A.block(0,3,2,3) = dz_dpfc * dpfc_dxtrans.block(0,0,3,3);
            A.block(0,6,2,3) = dz_dpfw;
            ...
            MatrixXd dh_dxcur = Eigen::Matrix<double,2,6>::Zero();
            dh_dxcur.block(0,0,2,3) = -A_opt.block(0,3,2,3);
            dh_dxcur.block(0,3,2,3) - -A_opt.block(0,0,2,3);
            
            MatrixXd dh_dpF = A_opt.block(0,6,2,3);
            ...
            // H_x.block(2*c,map_hx[tranform],2,tranform->size()).noalias() = dz_dpfc * dpfc_dxtrans; 
            H_x.block(2*c,map_hx[tranform],2,tranform->size()).noalias() = -dh_dxcur; 
            // }
...
void UpdaterHelper::get_feature_jacobian_ukf(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                            Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                            std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        double ts = state->_timestamp_approx; //ts 
        ...
        dpfw_dxnanchor.block(0,0,3,3) = skew_x(R_AnchorKFtoW*p_FinAnchorKF);
...
//        H_f.block(2*c,0,2,H_f.cols()).noalias() = dz_dpfw*dpfw_dpfk;
        ...
        // if(!state->iter)
        //compute H_x in the ukf way.
        const int L = 12; // reduced state dimension(cur state and transform)
        const int k = 2; // measurement dimension
        ...
        const double coff = std::sqrt(L+lambda);

        MatrixXd P=StateHelper::get_marginal_covariance(state,x_order);
        MatrixXd P_root(P.llt().matrixL());
        Matrix<double,L,2*L+1> dX = MatrixXd::Zero(L,2*L+1);
        dX.block<L,L>(0,1) = coff*P_root;
        dX.block<L,L>(0,L+1) = -coff*P_root; ///采样点
        Matrix<double,k,2*L+1> Z = MatrixXd::Zero(k,2*L+1); 
        for (int i = 0; i<2*L+1; i++)
        {
            const Matrix<double,L,1> dX_i = dX.block<L,1>(0,i); 
            Vector3d dq_cur=dX_i.block(0,0,3,1);
            Vector3d dp_cur=dX_i.block(3,0,3,1);
            Vector3d dq_trans=dX_i.block(6,0,3,1);
            Vector3d dp_trans=dX_i.block(9,0,3,1);
            Eigen::Matrix<double, 4, 1> dq;
            dq << .5 * dq_trans, 1.0;
            dq = ov_core::quatnorm(dq);
            Matrix3d R_GtoW_turb=quat_2_Rot(quat_multiply(dq,state->transform_vio_to_map->quat()));
            dq<<.5 * dq_cur, 1.0;
            dq = ov_core::quatnorm(dq);
            Matrix3d R_GtoI_turb=quat_2_Rot(quat_multiply(dq,state->_clones_IMU[state->_timestamp]->quat()));
            Vector3d p_GinW_turb=p_GinW_linp+dp_trans;
            Vector3d p_IinG_turb=p_IinG_linp+dp_cur;
            Vector3d p_FinG_turb = R_GtoW_turb.transpose()*(p_FinW-p_GinW_turb);
            Vector3d p_FinC_turb = R_ItoC*R_GtoI_turb*(p_FinG_turb-p_IinG_turb) + p_IinC;
            Matrix<double,2,1> uv_norm_turb;
            uv_norm_turb<<p_FinC_turb(0)/p_FinC_turb(2),p_FinC_turb(1)/p_FinC_turb(2);
            Eigen::Matrix<double,2,1> uv_dist_turb;
            if(state->_cam_intrinsics_model.at(0)) {

            // Calculate distorted coordinates for fisheye
            double r = std::sqrt(uv_norm_turb(0)*uv_norm_turb(0)+uv_norm_turb(1)*uv_norm_turb(1));
            double theta = std::atan(r);
            double theta_d = theta+cam_d(4)*std::pow(theta,3)+cam_d(5)*std::pow(theta,5)+cam_d(6)*std::pow(theta,7)+cam_d(7)*std::pow(theta,9);
            ...
            // Calculate distorted coordinates for fisheye
            double x1 = uv_norm_turb(0)*cdist;
            double y1 = uv_norm_turb(1)*cdist;
            uv_dist_turb(0) = cam_d(0)*x1 + cam_d(2);
            uv_dist_turb(1) = cam_d(1)*y1 + cam_d(3);
            ...
            double r_2 = r*r;
            double r_4 = r_2*r_2;
            double x1 = uv_norm_turb(0)*(1+cam_d(4)*r_2+cam_d(5)*r_4)+2*cam_d(6)*uv_norm_turb(0)*uv_norm_turb(1)+cam_d(7)*(r_2+2*uv_norm_turb(0)*uv_norm_turb(0));
            double y1 = uv_norm_turb(1)*(1+cam_d(4)*r_2+cam_d(5)*r_4)+cam_d(6)*(r_2+2*uv_norm_turb(1)*uv_norm_turb(1))+2*cam_d(7)*uv_norm_turb(0)*uv_norm_turb(1);
            uv_dist_turb(0) = cam_d(0)*x1 + cam_d(2);
            uv_dist_turb(1) = cam_d(1)*y1 + cam_d(3);

            }

             Z.block<k,1>(0,i)=uv_dist_turb;

        }

         Vector2d z_hat = Ws_0*Z.block<k,1>(0,0) + Ws_i*Z.rowwise().sum(); 

        for (int i = 0; i<2*L+1; i++){
            Z.block<k,1>(0,i) -= z_hat;     ///dz
        }

        Matrix<double,L,k> Pxz = Wc_i* dX*Z.transpose();
        H_x.block(2*c,0,2,L) = Pxz.transpose()*P.inverse();

        //ckf
        // double sigma=sqrt(2.0*L/2.0);
        // MatrixXd P=StateHelper::get_marginal_covariance(state,x_order);
        // MatrixXd P_root(P.llt().matrixL());
        // Matrix<double,L,2*L> dX = MatrixXd::Zero(L,2*L);
        // dX.block<L,L>(0,0) = sigma*P_root;
        // dX.block<L,L>(0,L) = -sigma*P_root; ///采样点
        // Matrix<double,k,2*L> Z = MatrixXd::Zero(k,2*L); 
        // for (int i = 0; i<2*L; i++)
        // {
        //     const Matrix<double,L,1> dX_i = dX.block<L,1>(0,i); 
        //     Vector3d dq_cur=dX_i.block(0,0,3,1);
        ...
        //     dq = ov_core::quatnorm(dq);
        //     Matrix3d R_GtoI_turb=quat_2_Rot(quat_multiply(dq,state->_clones_IMU[state->_timestamp]->quat()));
        //     Vector3d p_GinW_turb=p_GinW+dp_trans;
        //     Vector3d p_IinG_turb=p_IinG+dp_cur;
        //     Vector3d p_FinG_turb = R_GtoW_turb.transpose()*(p_FinW-p_GinW_turb);
        //     Vector3d p_FinC_turb = R_ItoC*R_GtoI_turb*(p_FinG_turb-p_IinG_turb) + p_IinC;
        //     Matrix<double,2,1> uv_norm_turb;
        //     uv_norm_turb<<p_FinC_turb(0)/p_FinC_turb(2),p_FinC_turb(1)/p_FinC_turb(2);
        ...
        //  Vector2d z_hat =  1/(2.0*L)*Z.rowwise().sum();  ///z_hat为z_k_k-1

        // for (int i = 0; i<2*L; i++){
        //     Z.block<k,1>(0,i) -= z_hat;     ///dz
        // }

        // Matrix<double,L,k> Pxz = 1/(2.0*L)* dX*Z.transpose();
        // H_x.block(2*c,0,2,L) = Pxz.transpose()*P.inverse();
        ...
        //now we finish the observation that reprojecting the 3d point in anchor kf to current frame;
        ...
        for(int i = 1;i<keyframe_id.size();i++)
        {
           ...
           Eigen::Vector3d p_FinKF = R_KFtoW.transpose()*(p_FinW-p_KFinW);
           Eigen::Vector3d p_FinKF_linp=R_KFtoW.transpose()*(p_FinW-p_KFinW);
           ...
           if(is_fisheye) {
            ...
            dpfk_dxnanchor.block(0,0,3,3)=R_KFtoW.transpose()*skew_x(R_AnchorKFtoW*p_FinAnchorKF);
            dpfk_dxnanchor.block(0,3,3,3)=R_KFtoW.transpose();

            H_n.block(2*c, map_hn[anchor_kf_w],2,anchor_kf_w->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dxnanchor;

            Matrix<double,3,6> dpfk_dxn=Matrix<double,3,6>::Zero();
            dpfk_dxn.block(0,0,3,3)=-R_KFtoW.transpose()*skew_x(p_FinW-p_KFinW);
