...
bool UpdaterSLAM::delayed_init_skf_with_KF(State *state, std::vector<Feature*>& feature_vec) {

    cout<<"in delayed_init_skf_with_KF"<<endl;
    // Return if no features
    if(feature_vec.empty())
        return false;
    ...
    while(it2 != feature_vec.end()) {
        ...
        max_meas_n_size += 2*feat.keyframe_matched_obs.size();
        ...
        Eigen::MatrixXd H_f_big;
        Eigen::MatrixXd H_x;
        Eigen::MatrixXd H_n;
        Eigen::MatrixXd H_n_x; //Jacobian of kf constrains with respect to x
        Eigen::MatrixXd H_x_big= Eigen::MatrixXd::Zero(max_meas_n_size+max_meas_size,max_hx_size);
        Eigen::MatrixXd H_n_big=Eigen::MatrixXd::Zero(max_meas_size+max_meas_n_size,max_hn_size);
        Eigen::VectorXd res_x;
        Eigen::VectorXd res_n;
        Eigen::VectorXd res_big;
        std::vector<Type*> Hx_order;
        std::vector<Type*> Hn_order;
        std::vector<Type*> Hnx_order;
        std::vector<Type*> Hx_big_order;
        std::vector<Type*> Hn_big_order;
        ...
        if(feat_rep==LandmarkRepresentation::Representation::ANCHORED_INVERSE_DEPTH_SINGLE) {
            ...
            Eigen::MatrixXd H_xf = H_x;
            H_xf.conservativeResize(H_x.rows(), H_x.cols()+1);
            H_xf.block(0, H_x.cols(), H_x.rows(), 1) = H_f_x.block(0,H_f_x.cols()-1,H_f_x.rows(),1);
            H_f_x.conservativeResize(H_f_x.rows(), H_f_x.cols()-1);


            Eigen::MatrixXd H_n_xf=H_n_x;
            H_n_xf.conservativeResize(H_n_x.rows(),H_n_x.cols()+1);
            H_n_xf.block(0,H_n_x.cols(),H_n_x.rows(),1)=H_f_n.block(0,H_f_n.cols()-1,H_f_n.rows(),1);
            H_f_n.conservativeResize(H_f_n.rows(),H_f_n.cols()-1);

            // Nullspace project the bearing portion
            // This takes into account that we have marginalized the bearing already
            // Thus this is crucial to ensuring estimator consistency as we are not taking the bearing to be true
            UpdaterHelper::nullspace_project_inplace(H_f_x, H_xf, res_x);

            UpdaterHelper::nullspace_project_inplace_with_nuisance(H_f_n,H_n_xf,H_n,res_n);

            // Split out the state portion and feature portion
            H_x = H_xf.block(0,0,H_xf.rows(),H_xf.cols()-1);
            H_f_x = H_xf.block(0,H_xf.cols()-1,H_xf.rows(),1);

            H_n_x=H_n_xf.block(0,0,H_n_xf.rows(),H_n_xf.cols()-1);
            H_f_n=H_n_xf.block(0,H_n_xf.cols()-1,H_n_xf.rows(),1);
        ...
        res_big=Eigen::MatrixXd::Zero(res_x.rows()+res_n.rows(),1);
        res_big.block(0,0,res_x.rows(),1)=res_x;
        res_big.block(res_x.rows(),0,res_n.rows(),1)=res_n;
        ...
        for(const auto &var : Hx_order) {
            ...
            if(Hx_mapping.find(var)==Hx_mapping.end()) {
                ...
                Hx_big_order.push_back(var);
                ct_jacob += var->size();
            }

            // Append to our large Jacobian
            H_x_big.block(ct_meas,Hx_mapping[var],H_x.rows(),var->size()) = H_x.block(0,ct_hx,H_x.rows(),var->size());
            ct_hx += var->size();
        ...
        for(const auto &var : Hnx_order){
            ...
            H_x_big.block(ct_meas,Hx_mapping[var],H_n_x.rows(),var->size())= H_n_x.block(0,ct_hnx,H_n_x.rows(),var->size());
            ct_hnx += var->size();
        }
        ...
        for(const auto &var : Hn_order){
            if(Hn_mapping.find(var)==Hn_mapping.end())
            {
                ...
                Hn_big_order.push_back(var);
                ct_jacob_n += var->size();
            }
            H_n_big.block(ct_meas,Hn_mapping[var],H_n.rows(),var->size()) = H_n.block(0,ct_hn,H_n.rows(),var->size());
            ct_hn += var->size();
        }

        ct_meas+=res_n.rows();

        assert(H_f_n.cols()==H_f_x.cols());
        assert(H_f_n.rows()+H_f_x.rows()==ct_meas);
        H_f_big=Eigen::MatrixXd::Zero(ct_meas,H_f_n.cols());
        H_f_big.block(0,0,H_f_x.rows(),H_f_x.cols())=H_f_x;
        H_f_big.block(H_f_x.rows(),0,H_f_n.rows(),H_f_n.cols())=H_f_n;

        assert(ct_meas<=max_meas_size+max_meas_n_size);
        assert(ct_jacob<=max_hx_size);
        assert(ct_jacob_n<=max_hn_size);
        res_big.conservativeResize(ct_meas,1);
        H_x_big.conservativeResize(ct_meas,ct_jacob);
        H_n_big.conservativeResize(ct_meas,ct_jacob_n);



        // Measurement noise matrix
        double sigma_pix_sq = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.sigma_pix_sq : _options_slam.sigma_pix_sq;
        Eigen::MatrixXd R = sigma_pix_sq*Eigen::MatrixXd::Identity(res_big.rows(), res_big.rows());

        // Try to initialize, delete new pointer if we failed
        double chi2_multipler = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.chi2_multipler : _options_slam.chi2_multipler;
        if (StateHelper::initialize_skf(state, landmark, Hx_big_order,Hn_big_order, H_x_big,H_n_big, H_f_big, R, res_big, chi2_multipler)) {
            state->_features_SLAM.insert({(*it2)->featid, landmark});
            (*it2)->to_delete = true;
            it2++;
        } else {
...
bool UpdaterSLAM::update_skf_with_KF(State *state, std::vector<Feature*>& feature_vec) {

    cout<<"in UpdaterSLAM::update_skf_with_KF"<<endl;
    // Return if no features
    if(feature_vec.empty())
        return false;
    ...
    // 4. Compute linear system for each feature, nullspace project, and reject
    cout<<"4"<<endl;
    cout<<"feature_vec.size(): "<<feature_vec.size()<<endl;
    auto it2 = feature_vec.begin();
    while(it2 != feature_vec.end()) {
        ...
        feat.timestamps = (*it2)->timestamps;
        feat.keyframe_matched_obs = (*it2)->keyframe_matched_obs;
        cout<<"finish feat construct"<<endl;
        ...
        // Get the Jacobian for this feature
        UpdaterHelper::get_feature_jacobian_full(state, feat, H_f_x, H_x, res_x, Hx_order);
        cout<<"finish get feature jacobian full"<<endl;

        UpdaterHelper::get_feature_jacobian_kf(state,feat,H_f_n,H_n,H_n_x,res_n,Hn_order,Hnx_order);
        cout<<"finish get feature jacobian kf"<<endl;
        ...
        if(landmark->_feat_representation==LandmarkRepresentation::Representation::ANCHORED_INVERSE_DEPTH_SINGLE) {
            ...
            H_nxf.conservativeResize(H_n_x.rows(),H_n_x.cols()+1);
            H_nxf.block(0, H_n_x.cols(), H_n_x.rows(), 1) = H_f_n.block(0,H_f_n.cols()-1,H_f_n.rows(),1);
            H_f_n.conservativeResize(H_f_n.rows(), H_f_n.cols()-1);
            ...
            // Thus this is crucial to ensuring estimator consistency as we are not taking the bearing to be true
            UpdaterHelper::nullspace_project_inplace(H_f_x, H_xf, res_x);
            UpdaterHelper::nullspace_project_inplace_with_nuisance(H_f_n,H_nxf,H_n,res_n);

        } else {

            // Else we have the full feature in our state, so just append it
            H_xf.conservativeResize(H_x.rows(), H_x.cols()+H_f_x.cols());
            H_xf.block(0, H_x.cols(), H_x.rows(), H_f_x.cols()) = H_f_x;

            H_nxf.conservativeResize(H_n_x.rows(),H_n_x.cols()+H_f_n.cols());
            H_nxf.block(0,H_n_x.cols(),H_n_x.rows(),H_f_n.cols())=H_f_n;

        }

        // Append to our Jacobian order vector
        std::vector<Type*> Hxf_order = Hx_order;
        std::vector<Type*> Hnxf_order=Hnx_order;
        Hxf_order.push_back(landmark);
        Hnxf_order.push_back(landmark);
        cout<<"finish Hxf_order"<<endl;
        /// Chi2 distance check
        Eigen::MatrixXd P_marg = StateHelper::get_marginal_covariance(state, Hxf_order);
        cout<<"get P_marg"<<endl;
        Eigen::MatrixXd P_n_marg=StateHelper::get_marginal_nuisance_covariance(state,Hn_order);
        cout<<"get P_n_marg"<<endl;
        Eigen::MatrixXd P_nx_marg=StateHelper::get_marginal_covariance(state,Hnxf_order);
        cout<<"get P_nx_marg"<<endl;
        Eigen::MatrixXd P_nxn_marg=StateHelper::get_marginal_cross_covariance(state,Hnxf_order,Hn_order);
        cout<<"get P_nxn_marg"<<endl;

        Eigen::MatrixXd S1= H_xf*P_marg*H_xf.transpose();

        Eigen::MatrixXd S2(H_n.rows(),H_n.rows());
        S2.triangularView<Eigen::Upper>()= H_n*P_n_marg*H_n.transpose();
        S2.triangularView<Eigen::Upper>()+=H_nxf*P_nx_marg*H_nxf.transpose();
        S2+=H_n*P_nxn_marg.transpose()*H_nxf.transpose();
        S2+=H_nxf*P_nxn_marg*H_n.transpose();

        double sigma_pix_sq = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.sigma_pix_sq : _options_slam.sigma_pix_sq;
        S1.diagonal() += sigma_pix_sq*Eigen::VectorXd::Ones(S1.rows());
        S2.diagonal() += sigma_pix_sq*Eigen::VectorXd::Ones(S2.rows());
        cout<<"finish S1 S2"<<endl;

        double chi2_x = res_x.dot(S1.llt().solve(res_x));
        double chi2_n = res_n.dot(S2.llt().solve(res_n));


        // Get our threshold (we precompute up to 500 but handle the case that it is more)
        double chi2_check_1,chi2_check_2;
        if(res_x.rows() < 500) {
            chi2_check_1 = chi_squared_table[res_x.rows()];
        } else {
            boost::math::chi_squared chi_squared_dist(res_x.rows());
            chi2_check_1 = boost::math::quantile(chi_squared_dist, 0.95);
            printf(YELLOW "chi2_check over the residual limit - %d\n" RESET, (int)res_x.rows());
        }
        if(res_n.rows() < 500) {
        ...
        // Check if we should delete or not
        double chi2_multipler = ((int)feat.featid < state->_options.max_aruco_features)? _options_aruco.chi2_multipler : _options_slam.chi2_multipler;
        if(chi2_x > chi2_multipler*chi2_check_1) {
            if((int)feat.featid < state->_options.max_aruco_features)
                printf(YELLOW "[SLAM-UP]: rejecting aruco tag %d for chi2 thresh (%.3f > %.3f)\n" RESET,(int)feat.featid,chi2_x,chi2_multipler*chi2_check_1);
            (*it2)->to_delete = true;
            it2 = feature_vec.erase(it2);
            continue;
        }
        else if(chi2_n > chi2_multipler*chi2_check_2)
        {
            if((int)feat.featid < state->_options.max_aruco_features)
                printf(YELLOW "[SLAM-UP]: rejecting aruco tag %d for chi2 thresh (%.3f > %.3f)\n" RESET,(int)feat.featid,chi2_n,chi2_multipler*chi2_check_2);
            (*it2)->to_delete = true;
            it2 = feature_vec.erase(it2);
            continue;
        }
        ...
        // Debug print when we are going to update the aruco tags
        if((int)feat.featid < state->_options.max_aruco_features)
            printf("[SLAM-UP]: accepted aruco tag %d for chi2 thresh (%.3f < %.3f)\n",(int)feat.featid,chi2_x,chi2_multipler*chi2_check_1);

        // We are good!!! Append to our large H vector
        cout<<"before Hx_big with Hxf_order"<<endl;
        size_t ct_hx = 0;
        for(const auto &var : Hxf_order) {
        ...
        // Our isotropic measurement noise
        R_big.block(ct_meas,ct_meas,res_x.rows(),res_x.rows()) *= sigma_pix_sq;

        // Append our residual and move forward
        res_big.block(ct_meas,0,res_x.rows(),1) = res_x;
        ct_meas += res_x.rows();

        //Append H_nxf into the Hx_big
        cout<<"before Hx_big with Hnxf_order"<<endl;
        size_t ct_hnxf = 0;
        for(const auto &var : Hnxf_order){
            if(Hx_mapping.find(var)==Hx_mapping.end())
            {
                Hx_mapping.insert({var,ct_jacob});
                Hx_order_big.push_back(var);
            ...
            Hx_big.block(ct_meas,Hx_mapping[var],H_nxf.rows(),var->size())= H_nxf.block(0,ct_hnxf,H_nxf.rows(),var->size());
            ct_hnxf += var->size();
        }

        //Append H_n into Hn_big
        cout<<"before Hn_big with Hx_order"<<endl;
        size_t ct_hn =0;
        for(const auto &var : Hn_order){
            if(Hn_mapping.find(var)==Hn_mapping.end())
            {
        ...
        R_big.block(ct_meas,ct_meas,res_n.rows(),res_n.rows()) *= sigma_pix_sq;
