 ...
 */
#ifndef OV_RIMSCKF_STATE_HELPER_H
#define OV_RIMSCKF_STATE_HELPER_H
...
namespace ov_rimsckf {
    ...
    public:
        ...
        /**
         * @brief Performs SKF update of the state 
         * @param state Pointer to state
         * @param H_order Variable ordering used in the compressed Jacobian
         * @param H Condensed Jacobian of updating measurement
         * @param res residual of updating measurement
         ...
         */
         ///added by zzq
        static void SKFUpdate(State *state, const std::vector<Type *> &Hx_order,const std::vector<Type *> &Hn_order, const Eigen::MatrixXd &Hx,const Eigen::MatrixXd &H_nuisance,
                              const Eigen::VectorXd &res, Eigen::MatrixXd &R, bool iterative);

        static void init_transform_update(State *state, const std::vector<Type *> &Ha_order, const std::vector<Type *> &Ht_order, const std::vector<Type *> &Hn_order,
         ...
         /**
         * @brief Performs EKF update of the state with map informatino 
         * @param state Pointer to state
         * @param H_order Variable ordering used in the compressed Jacobian
         * @param H Condensed Jacobian of updating measurement
         * @param res residual of updating measurement
         ...
         */
         ///added by zzq
        static void EKFMAPUpdate(State *state, const std::vector<Type *> &Hx_order,const std::vector<Type *> &Hn_order, const Eigen::MatrixXd &Hx,const Eigen::MatrixXd &H_nuisance,
                              const Eigen::VectorXd &res, const Eigen::MatrixXd &R, bool iterative);


        /**
         * @brief  Performs variable update in the form of invariant EKF
         * @param state Pointer to state
         * @param dx delta computed by EKF
         */
        static void UpdateVarInvariant(State *state, const Eigen::VectorXd dx);

        /**
         * @brief construct variables in the form of SE_{2+N}^{1}(3)
         * the related variable would be R_IinG, p_IinG, v_IinG, landmark_features, relative transformation
         * @param state Pointer to state
         * @return Eigen::MatrixXd 
         */
        static Eigen::MatrixXd ConstructSEN(State *state);

        /**
         * @brief Recover the state from updated X_SEN
         * the related state would be R_IinG, p_IinG, v_IinG, landmark_features, relative transformation
         * @param state Pointer to state
         * @param X_SEN updated state in the form of SE_{2+N}^{1}(3)
         */
        static void SeparateSEN(State *state, Eigen::MatrixXd X_SEN);

        /**
         * @brief Update clone poses in the form of SE3
         * 
         * @param state Pointer to state 
         * @param dx delta computed by EKF
         */
        static void UpdateClonePose(State *state, const Eigen::VectorXd dx);

        /**
         * @brief Update Paramters including bg,ba,dt,extrinsics,intrinsics,time_delay
         * 
         * @param state Pointer to state
         * @param dx delta computed by EKF
         */
        static void UpdateParameters(State *state, const Eigen::VectorXd dx);

         
        /**
         * @brief select sub-vector from dx
         * sub-vector corresponding to the variables used to construct SE_{2+N}^{1}(3)
         * @param state Pointer to state
         * @param dx delta computed by EKF
         * @return Eigen::VectorXd 
         */
        static Eigen::VectorXd Construct_dxSEN(State *state, const Eigen::VectorXd dx);
        ...
        */
        ...
        static Eigen::MatrixXd get_marginal_nuisance_covariance(State *state, const std::vector<Type *> &small_variables);

        static Eigen::MatrixXd get_marginal_cross_covariance(State *state, const std::vector<Type *> &x_variables,const std::vector<Type *> &n_variables);
        ...
        static bool initialize_skf(State *state, Type *new_variable, const std::vector<Type *> &Hx_order,const std::vector<Type *> &Hn_order, Eigen::MatrixXd &H_x,
                                   Eigen::MatrixXd &H_n, Eigen::MatrixXd &H_f, Eigen::MatrixXd &R, Eigen::VectorXd &res, double chi_2_mult);
        ...
        static void initialize_invertible_with_nuisance(State *state, Type *new_variable, const std::vector<Type *> &Hx_order, const std::vector<Type *> &Hn_order,const Eigen::MatrixXd &H_x,
                                         const Eigen::MatrixXd &H_n, const Eigen::MatrixXd &H_f, const Eigen::MatrixXd &R, const Eigen::VectorXd &res);
        ...
        static void add_kf_nuisance(State* state,Keyframe *kf);

        static bool add_map_transformation(State* state,Matrix<double,7,1> pose_map_to_vio);

        static bool reset_map_transformation(State* state,Matrix<double,7,1> pose_vio_to_map);
...
#endif //OV_RIMSCKF_STATE_HELPER_H
