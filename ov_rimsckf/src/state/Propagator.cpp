...
//consider augmented variable noise
void Propagator::propagate_and_clone2(State* state, double timestamp) {
    ...
    //compute the size of propagation matrix. For RI-MSCKF, the propagation matrix
    //should be related to R_G_I, p_G_I, v_G_I, bg,ba, landmark_features, 6 DOF features( both translation and orientation are considered);
    int size_imu = state->_imu->size();
    assert(size_imu==15);
    int size_landmarks = state->_features_SLAM.size() * 3;
    ...
    int size_6DOFfeatures = 0;
    if(state->set_transform)
        size_6DOFfeatures = 1 * 6; //for localization, we only have 1 6DOF feature, i.e. transformation between local and global reference
    ...
    if(prop_data.size() > 1) {
        for(size_t i=0; i<prop_data.size()-1; i++) {
            ...
            boost::posix_time::ptime t1 = boost::posix_time::microsec_clock::local_time();
            predict_and_compute2(state, prop_data.at(i), prop_data.at(i+1), F, Qdi);
            //Here F is equal to Phi in documents. Qdi is equal to G_k*Q_d*G_k^T in documents
            boost::posix_time::ptime t2 = boost::posix_time::microsec_clock::local_time();
            // printf("predict and compute time: %f s\n", (t2-t1).total_microseconds() * 1e-6);
    ...
    if(state->set_transform)
    {
        Phi_order.push_back(state->transform_map_to_vio);
    }
    boost::posix_time::ptime t3 = boost::posix_time::microsec_clock::local_time();
    cout<<"Phi_order size: "<<Phi_order.size()<<endl;
    StateHelper::EKFPropagation2(state, Phi_order, Phi_order, Phi_summed, Qd_summed);
    boost::posix_time::ptime t4 = boost::posix_time::microsec_clock::local_time();
    printf("EKFPropagation time: %f s\n", (t4-t3).total_microseconds() * 1e-6);
...
//     // Corrected imu measurements
//     //TODO:why a_hat doesn't minus the influence of gravity?
...
//         F.block(p_id, p_id, 3, 3).setIdentity();F.setZero();
...
//     Qc.block(6,6,3,3) = _noises.sigma_wb_2/dt*Eigen::Matrix<double,3,3>::Identity(); //note that G.block(bg_id,6,3,3) = dt*Identity();
...
void Propagator::predict_and_compute(State *state, const IMUDATA data_minus, const IMUDATA data_plus,
    ...
    // Allocate noise Jacobian
    Eigen::MatrixXd W = Eigen::MatrixXd::Zero(prop_size,prop_size);
    ...
    W.block(th_id,th_id,3,3) = R_ItoG;
    W.block(p_id,th_id,3,3) = skew_x(state->_imu->pos())*R_ItoG;
    W.block(p_id,p_id,3,3) = R_ItoG;
    W.block(v_id,th_id,3,3) = skew_x(state->_imu->vel())*R_ItoG;
    W.block(v_id,v_id,3,3) = R_ItoG;

    W.block(bg_id,bg_id,3,3) = Eigen::Matrix3d::Identity();
    W.block(ba_id,ba_id,3,3) = Eigen::Matrix3d::Identity(); 
    ...
    if(state->_features_SLAM.size()>0)
    {
        ...
        while(iter != state->_features_SLAM.end())
        {
            ...
            W.block(landmark_id,th_id,3,3) = skew_x(iter->second->get_xyz(false))*R_ItoG;
            W.block(landmark_id,landmark_id,3,3) = R_ItoG;
    ...
    if(state->set_transform)
    {
        ...
        W.block(f6dof_id,th_id,3,3) = skew_x(state->transform_map_to_vio->pos())*R_ItoG;
        W.block(f6dof_id,f6dof_id,3,3) = R_ItoG;
    ...
    Eigen::MatrixXd Qc = Eigen::MatrixXd::Zero(prop_size,prop_size);
    Qc.block(th_id,th_id,3,3) = _noises.sigma_w_2*Eigen::Matrix<double,3,3>::Identity();
    Qc.block(v_id,v_id,3,3) = _noises.sigma_a_2*Eigen::Matrix<double,3,3>::Identity();
    Qc.block(bg_id,bg_id,3,3) = _noises.sigma_wb_2*Eigen::Matrix<double,3,3>::Identity();
    Qc.block(ba_id,ba_id,3,3) = _noises.sigma_ab_2*Eigen::Matrix<double,3,3>::Identity();
    ...
    F = Eigen::MatrixXd::Identity(prop_size,prop_size) + Adt + 0.5*Adt_square + (1.0/6.0)*Adt_cube;
    ...
    //Now replace imu estimate and fej with propagated values
    cout<<" in predict and compute: "<<endl;
     printf("bg = %.4f,%.4f,%.4f | ba = %.4f,%.4f,%.4f\n",
             state->_imu->bias_g()(0),state->_imu->bias_g()(1),state->_imu->bias_g()(2),
             state->_imu->bias_a()(0),state->_imu->bias_a()(1),state->_imu->bias_a()(2));
    Eigen::Matrix<double,16,1> imu_x = state->_imu->value();
    cout<<"imu_x: "<<imu_x.block(10,0,3,1).transpose()<<" "<<imu_x.block(13,0,3,1).transpose()<<endl;
    imu_x.block(0,0,4,1) = new_q;
    imu_x.block(4,0,3,1) = new_p;
    imu_x.block(7,0,3,1) = new_v;
    state->_imu->set_value(imu_x);
...
//consider augment variable noise
void Propagator::predict_and_compute2(State *state, const IMUDATA data_minus, const IMUDATA data_plus,
                                     Eigen::MatrixXd &F, Eigen::MatrixXd &Qd) {
    ...
    if(state->set_transform)
    {
      W = Eigen::MatrixXd::Zero(prop_size,3*6);
    }
    Eigen::MatrixXd A = Eigen::MatrixXd::Zero(prop_size,prop_size);
    Eigen::MatrixXd A2 = Eigen::MatrixXd::Zero(prop_size,prop_size); //A*A
    Eigen::MatrixXd A3 = Eigen::MatrixXd::Zero(prop_size,prop_size);  //A*A*A
    ...
    if(!state->_options.use_gt)
    {
      ...
      if(state->set_transform)
      {
         W.block(bg_id,12,3,3) = Eigen::Matrix3d::Identity();
         W.block(ba_id,15,3,3) = Eigen::Matrix3d::Identity(); 
      }
      else
      {
         W.block(bg_id,6,3,3) = Eigen::Matrix3d::Identity();
      ...
      if(state->set_transform)
      {
          int f6dof_rot_id = landmark_id;
          int f6dof_translation_id = f6dof_rot_id+state->transform_map_to_vio->q()->size();
          A.block(f6dof_translation_id,bg_id,3,3) = -skew_x(state->transform_map_to_vio->pos())*R_ItoG;
          W.block(f6dof_translation_id,0,3,3) = skew_x(state->transform_map_to_vio->pos())*R_ItoG;
          W.block(f6dof_translation_id,9,3,3) = R_ItoG;
          W.block(f6dof_rot_id,6,3,3) = state->transform_map_to_vio->Rot();
          landmark_id += state->transform_map_to_vio->size();
      }
      total_size = landmark_id;
      assert(total_size==prop_size);
    }
    ...
    if(state->set_transform)
    {
      Qc = Eigen::MatrixXd::Zero(18,18);
      Qc.block(0,0,3,3) = _noises.sigma_w_2*Eigen::Matrix<double,3,3>::Identity();
      Qc.block(3,3,3,3) = _noises.sigma_a_2*Eigen::Matrix<double,3,3>::Identity();
      Qc.block(6,6,3,3) = _noises.sigma_avr_2*Eigen::Matrix<double,3,3>::Identity();
      Qc.block(9,9,3,3) = _noises.sigma_avp_2*Eigen::Matrix<double,3,3>::Identity();
      Qc.block(12,12,3,3) = _noises.sigma_wb_2*Eigen::Matrix<double,3,3>::Identity();
      Qc.block(15,15,3,3) = _noises.sigma_ab_2*Eigen::Matrix<double,3,3>::Identity();
      // cout<<"avr_noise_2: "<<_noises.sigma_avr_2<<" avp_noise_2: "<<_noises.sigma_avp_2<<endl;
    }
    else
    {
      Qc.block(0,0,3,3) = _noises.sigma_w_2*Eigen::Matrix<double,3,3>::Identity();
