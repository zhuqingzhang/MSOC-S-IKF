...
namespace ov_eval {
    ...
    protected:
        ...
        /**
         * @brief Align estimate to GT using only position and yaw (for gravity aligned trajectories) using a set of initial poses, after align, the trajectories are moved to the same origin point
         * @param traj_es Estimated trajectory values in estimate frame [pos,quat]
         * @param traj_gt Groundtruth trjaectory in groundtruth frame [pos,quat]
         * @param R Rotation from estimate to GT frame that will be computed
         * @param t translation from estimate to GT frame that will be computed
         * @param n_aligned Number of poses to use for alignment (-1 will use all)
         */
        static void align_posyaworigin(const std::vector<Eigen::Matrix<double,7,1>> &traj_es, const std::vector<Eigen::Matrix<double,7,1>> &traj_gt,
                                 Eigen::Matrix3d &R, Eigen::Vector3d &t, int n_aligned = -1);                         
        ...
        /**
         * @brief Align estimate to GT using a full SE(3) transform using a set of initial poses,after align, the trajectories are moved to the same origin point
         * @param traj_es Estimated trajectory values in estimate frame [pos,quat]
         * @param traj_gt Groundtruth trjaectory in groundtruth frame [pos,quat]
         * @param R Rotation from estimate to GT frame that will be computed
         * @param t translation from estimate to GT frame that will be computed
         * @param n_aligned Number of poses to use for alignment (-1 will use all)
         */
        static void align_se3origin(const std::vector<Eigen::Matrix<double,7,1>> &traj_es, const std::vector<Eigen::Matrix<double,7,1>> &traj_gt,
                              Eigen::Matrix3d &R, Eigen::Vector3d &t, int n_aligned = -1);
