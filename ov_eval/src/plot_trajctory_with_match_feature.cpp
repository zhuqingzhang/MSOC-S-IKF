...
void plot_xy_points(const std::string &color, const std::vector<Eigen::Vector3d> &positions, const std::vector<Eigen::Vector3d> &points)
{
    std::map<std::string, std::string> params;
    // params.insert({"linestyle", "-"});
    params.insert({"marker","o"});
    params.insert({"color", color});

    // Create vectors of our x and y axis
    std::vector<double> x, y;
    for(size_t i=0; i<positions.size(); i++) {
        x.push_back(positions.at(i)(0));
        y.push_back(positions.at(i)(1));
    }

    // Finally plot
    // matplotlibcpp::plot(x, y, params);
    matplotlibcpp::scatter(x,y,params);
}
...
int main(int argc, char **argv) {
    ...
    if(argc != 5) {
        ...
        printf(RED "ERROR: ./plot_trajectories <align_mode> <file_gt.txt> <file_est1.txt> <file_point.txt>\n" RESET);
        printf(RED "ERROR: rosrun ov_eval plot_trajectories <align_mode> <file_gt.txt> <file_est1.txt>  <file_point.txt>\n" RESET);
    ...
    std::vector<Eigen::Vector3d> points_align;
    std::vector<Eigen::Vector3d> positions_align;
    std::vector<Eigen::Vector3d> points;
    std::vector<Eigen::Vector3d> positions;
    ov_eval::Loader::load_points(argv[4],points,positions);
    for(int i=2; i<4; i++) {
        ...
        if(i>2) {
            ...
            {
                Eigen::Vector3d point_tmp,position_tmp;
                position_tmp=s_ESTtoGT*R_ESTtoGT*positions[j]+t_ESTinGT-gt_est_origin;
                point_tmp=s_ESTtoGT*R_ESTtoGT*points[j]+t_ESTinGT-gt_est_origin;
                points_align.push_back(point_tmp);
                positions_align.push_back(position_tmp);
            }
    ...
    for(size_t i=0; i<times.size(); i++) {
     ...
     plot_xy_points("red",positions_align,points_align);
