...
ResultTrajectory::ResultTrajectory(std::string path_est, std::string path_gt, std::string alignment_method) {
    ...
    AlignUtils::perform_association(0, 0.02,
                                    est_times, gt_times, est_poses, gt_poses,
                                    est_covori, est_covpos, gt_covori, gt_covpos);
...
void ResultTrajectory::calculate_ate(Statistics &error_ori, Statistics &error_pos) {
    ...
    // Calculate the position and orientation error at every timeste
...
void ResultTrajectory::calculate_rpe_2d(const std::vector<double> &segment_lengths, std::map<double,std::pair<Statistics,Statistics>> &error_rpe) {
    ...
    accum_distances[0] = 0;
    for (size_t i = 1; i < gt_poses.size(); i++) {
        double pos_diff = (gt_poses[i].block(0,0,2,1) - gt_poses[i - 1].block(0,0,2,1)).norm();
        accum_distances[i] = accum_distances[i - 1] + pos_diff;
        average_pos_difference += pos_diff;
    }
    average_pos_difference /= gt_poses.size();
    ...
    for(const double &distance : segment_lengths) {
        ...
        for (size_t id_start = 0; id_start < comparisons.size(); id_start++) {
            ...
            // Compute error for position
            error_pos.timestamps.push_back(est_times.at(id_start));
            error_pos.values.push_back(T_error_in_w.block(0, 3, 2, 1).norm());

            // Calculate orientation error
            double ori_err = 180.0/M_PI*Math::log_so3(T_error_in_w.block(0, 0, 3, 3))(2);
            error_ori.timestamps.push_back(est_times.at(id_start));
            error_ori.values.push_back(ori_err);
...
void ResultTrajectory::calculate_invariant_nees(Statistics &nees_ori, Statistics &nees_pos) {
    ...
    for(size_t i=0; i<est_poses.size(); i++) {
        ...
        // Calculate orientation error
        // NOTE: we define our error as e_R = -Log(Rhat*R^T) R should be R_ItoG
        //for est_poses, it stores q_ItoG in Hamlton format or q_GtoI in JPL format
        Eigen::Matrix3d e_R =Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose() * Math::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3,0,4,1)) ;
        ...
        Eigen::Vector3d errpos = est_poses.at(i).block(0,0,3,1) - e_R * gt_poses_aignedtoEST.at(i).block(0,0,3,1);
...
void ResultTrajectory::calculate_nees(Statistics &nees_ori, Statistics &nees_pos) {
    ...
    for(size_t i=0; i<est_poses.size(); i++) {
        ...
        Eigen::Matrix3d e_R = Math::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3,0,4,1)) * Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose();
        Eigen::Vector3d errori = -Math::log_so3(e_R);
        //Eigen::Vector4d e_q = Math::quat_multiply(gt_poses_aignedtoEST.at(i).block(3,0,4,1),Math::Inv(est_poses.at(i).block(3,0,4,1)));
        //Eigen::Vector3d errori = 2*e_q.block(0,0,3,1);
        double ori_nees = errori.transpose()*est_covori.at(i).inverse()*errori;
...
void ResultTrajectory::calculate_inv_nees(Statistics &nees_ori, Statistics &nees_pos) {
    ...
    for(size_t i=0; i<est_poses.size(); i++) {
        ...
        // Calculate orientation error
        // note that quat_2_Rot(est_pose) get the rotation from World to IMU
        Eigen::Matrix3d e_R =  Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose() * Math::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3,0,4,1));
        Eigen::Vector3d e_p = est_poses.at(i).block(0,0,3,1)-e_R*gt_poses_aignedtoEST.at(i).block(0,0,3,1);
        Eigen::Matrix4d e=Eigen::Matrix4d::Identity();
        e.block(0,0,3,3) = e_R;
        e.block(0,3,3,1) = e_p;
        Eigen::VectorXd error_vec = Math::log_se3(e);
        
        
        Eigen::Vector3d errori = error_vec.block(0,0,3,1);
        Eigen::Vector3d errpos = error_vec.block(3,0,3,1);
        //Eigen::Vector4d e_q = Math::quat_multiply(gt_poses_aignedtoEST.at(i).block(3,0,4,1),Math::Inv(est_poses.at(i).block(3,0,4,1)));
        //Eigen::Vector3d errori = 2*e_q.block(0,0,3,1);
        double ori_nees = errori.transpose()*est_covori.at(i).inverse()*errori;
 ...
 void ResultTrajectory::calculate_invariant_transform_nees(std::vector<Eigen::Matrix<double,7,1>>& est_traj_poses,std::vector<Eigen::Matrix<double,7,1>>& gt_traj_poses,
                                                Statistics &nees_ori, Statistics &nees_pos)
{
 // Check that we have our covariance matrices to normalize with
    if(est_times.size() != est_covori.size() || est_times.size() != est_covpos.size()
        || gt_times.size() != gt_covori.size() || gt_times.size() != gt_covpos.size()) {
    ...
    for(size_t i=0; i<est_poses.size(); i++) {
        ...
        // note that quat_2_Rot(est_pose) get the rotation from World to IMU
        Eigen::Matrix3d e_R =  Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose() * Math::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3,0,4,1));
        Eigen::Matrix3d e_R_traj = Math::quat_2_Rot(est_traj_poses.at(i).block(3,0,4,1)).transpose() * Math::quat_2_Rot(gt_traj_poses.at(i).block(3,0,4,1));
        Eigen::Vector3d e_p = est_poses.at(i).block(0,0,3,1)-e_R_traj*gt_poses_aignedtoEST.at(i).block(0,0,3,1);
        Eigen::Matrix4d e=Eigen::Matrix4d::Identity();
        e.block(0,0,3,3) = e_R_traj;
        e.block(0,3,3,1) = e_p;
        Eigen::VectorXd error_vec = Math::log_se3(e);
...
void ResultTrajectory::calculate_invariant_error(Statistics &posx, Statistics &posy, Statistics &posz,
                                       Statistics &orix, Statistics &oriy, Statistics &oriz,
                                       Statistics &roll, Statistics &pitch, Statistics &yaw) {
    ...
    for(size_t i=0; i<est_poses.size(); i++) {
        ...
        // Calculate local orientation error, then propagate its covariance into the global frame of reference
        //for invariant, we care about R_ItoG not R_GtoI. e_q is global error, we do not have to convert it from local to global
        Eigen::Matrix3d e_R =  Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose() * Math::quat_2_Rot(gt_poses_aignedtoEST.at(i).block(3,0,4,1));
        Eigen::Vector3d e_p = est_poses.at(i).block(0,0,3,1)-e_R*gt_poses_aignedtoEST.at(i).block(0,0,3,1);
        Eigen::Matrix4d e=Eigen::Matrix4d::Identity();
        e.block(0,0,3,3) = e_R;
        ...
        Eigen::Vector3d errori = error_vec.block(0,0,3,1);
        Eigen::Vector3d errpos = error_vec.block(3,0,3,1);
        // errorpos = gt_poses_aignedtoEST.at(i).block(0,0,3,1) - est_poses.at(i).block(0,0,3,1);

        Eigen::Vector3d errori_global = errori;
        Eigen::Matrix3d cov_global = est_covori.at(i);
        // Eigen::Vector3d errori_local = 2*e_q.block(0,0,3,1);
        // Eigen::Vector3d errori_global = Math::quat_2_Rot(est_poses.at(i).block(3,0,4,1)).transpose()*errori_local;
        // Eigen::Matrix3d cov_global;
        // if(est_times.size() == est_covori.size()) {
 ...
 void ResultTrajectory::calculate_invariant_transform_error(std::vector<Eigen::Matrix<double,7,1>>& est_traj_poses,std::vector<Eigen::Matrix<double,7,1>>& gt_traj_poses,
                                                Statistics &posx,Statistics &posy,Statistics &posz,Statistics &orix,Statistics &oriy,Statistics &oriz){
     // Clear any old data
    posx.clear();
    posy.clear();
    posz.clear();
