...
void TrackDescriptor::feed_monocular(double timestamp, cv::Mat &imgin, size_t cam_id) {
    ...
    //extract fast feature and compute its orb descriptor
    ...
    for(size_t i=0; i<pts_new.size(); i++) {
        ...
        // If we found a good  track from last to current
...
void TrackDescriptor::feed_monocular_2(double timestamp, vector<map<double,vector<VectorXd>>>& matches,size_t cam_id)
{
     // Start timing
    // rT1 =  boost::posix_time::microsec_clock::local_time();
...
void TrackDescriptor::feed_stereo(double timestamp, cv::Mat &img_leftin, cv::Mat &img_rightin, size_t cam_id_left, size_t cam_id_right) {
    ...
    // Start timing
    std::cout <<"in TrackDescriptor::feed_stereo"<< std::endl;
    rT1 =  boost::posix_time::microsec_clock::local_time();
    ...
    // First, extract new descriptors for this new image
    //extract fast feature and compute orb descriptor on left and right images.
    //then, the features on these two images are matches trough robust_match function,
    //after perform_detection_stereo, the pts_left_new and pts_right_new have the size size and correspond to the same feature.
    ...
    // Lets match temporally
    std::cout <<"in TrackDescriptor::feed_stereo before temporally robust_match1"<< std::endl;
    ...
    std::cout <<"in TrackDescriptor::feed_stereo before temporally robust_match2"<< std::endl;
    ...
    std::cout <<"in TrackDescriptor::feed_stereo after temporally robust_match1"<< std::endl;
    ...
    //left and right image must have same number of features, as they have already been matched before
    ...
    for(size_t i=0; i<pts_left_new.size(); i++) {
        ...
        // Loop through all right matches, and find the old "train" id
        //as pts_left_new's elements correspond to pts_right_new's elements,
        //so we don't need to loop through pts_right_new, just in the for-loop of pts_left_new is ok
        int idrr = -1;
        ...
        if(idll != -1 && idrr != -1 && ids_last[cam_id_left][idll] == ids_last[cam_id_right][idrr]) {
            ...
            //to be precisely, it should be ids_right_new[i].
            // however, from perform_detection_stereo function, ids_left_new is identical to ids_right_new.
...
void TrackDescriptor::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1,
    ...
    std::cout <<"in TrackDescriptor::perform_detection_stereo"<< std::endl;
    // Assert that we need features
    assert(pts0.empty());
    assert(pts1.empty());
...
void TrackDescriptor::robust_match(std::vector<cv::KeyPoint>& pts0, std::vector<cv::KeyPoint> pts1,
    ...
    std::cout <<"in TrackDescriptor::robust_match"<< std::endl;
