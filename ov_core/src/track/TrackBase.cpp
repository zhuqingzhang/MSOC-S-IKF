...
void TrackBase::display_history(cv::Mat &img_out, int r1, int g1, int b1, int r2, int g2, int b2) {
    ...
    //size_t maxtracks = 10;
    size_t maxtracks = (size_t)-1; //超级大的一个数
...
void TrackBase::add_features_into_database(double timestamp, size_t cam_id ,std::vector<cv::Point2f> &uv, std::vector<size_t> &id)
{
    
    for(size_t i=0; i<uv.size(); i++) {
        // move id foward and append this new point
        // cout<<"sim_feats_num: "<<sim_feats_num<<endl;
        if(sim_feats_num==-1)
        {
            size_t temp = ++currid;
            id.push_back(temp);
        ...
        {
            sim_feats_num++;
            size_t temp = currid+sim_feats_num;
            id.push_back(temp);
        }
    ...
    for(size_t i=0; i<uv.size(); i++) {
        cv::Point2f npt_l = undistort_point(uv[i], cam_id);
        database->update_feature(id[i], timestamp, cam_id,
                                 uv[i].x, uv[i].y,
                                 npt_l.x, npt_l.y);
        database->get_feature(id[i])->new_extracted_map_match=true;
    }
}
