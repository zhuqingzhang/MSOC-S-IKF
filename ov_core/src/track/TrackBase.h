    ...
    public:
        ...
        void set_calibration(std::map<size_t,Eigen::VectorXd> camera_calib,
            ...
            if(mtx_feeds.empty() || camera_k_OPENCV.empty() || camera_k_OPENCV.empty() || this->camera_fisheye.empty()) {
                ...
                std::vector<std::mutex> list(camera_calib.size());  //the size is 2 at most
                mtx_feeds.swap(list);
        ...
        void add_features_into_database(double timestamp, size_t cam_id ,std::vector<cv::Point2f> &uv, std::vector<size_t> &id);

        inline void set_sim_feats_num(int n){sim_feats_num=n;}
