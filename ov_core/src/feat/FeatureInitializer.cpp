...
bool FeatureInitializer::single_triangulation(Feature* feat, std::unordered_map<size_t,std::unordered_map<double,ClonePose>> &clonesCAM) {
    ...
    size_t most_meas = 0;
    for (auto const& pair : feat->timestamps) {
        total_meas += (int)pair.second.size(); //通过时间戳的个数来获得cam0/cam1 (pair.first)对当前feature的观测数
        if(pair.second.size() > most_meas) { //对比cam0和cam1对当前feature的观测数(pair.second.size()),选择观测数多的那个相机作为anchor  //by zzq
            anchor_most_meas = pair.first;
            most_meas = pair.second.size();
        }
    }
    feat->anchor_cam_id = anchor_most_meas;
    //在初始化特征点时选择最新的观测到特征点的时间戳作为anchor_clone_timestamp,不是最老的观测？ //by zzq
    ...
    //condition number: max_eigenvalue/min_eigencalue. by zzq
        ...
        if (recompute) {
            ...
            for (auto const& pair : feat->timestamps) {
                ...
                for (size_t m = 0; m < feat->timestamps.at(pair.first).size(); m++) {
                    ...
                    // Middle variables of the system
                    //p_finA=[alpha,beta,1]*(1/rho)
                    //p_finC=R_AtoC*p_finA+p_AinC---> rho*p_finC=R_AtoC*[alpha,beta,1]+rho*p_AinC
                    //rho*p_finC == [hi1,hi2,hi3]
                    //note that there is a little difference in the experssion of hi1,hi2,hi3 between here and internet pages. by zzq
                    double hi1 = R_AtoCi(0, 0) * alpha + R_AtoCi(0, 1) * beta + R_AtoCi(0, 2) + rho * p_AinCi(0, 0);
                    double hi2 = R_AtoCi(1, 0) * alpha + R_AtoCi(1, 1) * beta + R_AtoCi(1, 2) + rho * p_AinCi(1, 0);
                    double hi3 = R_AtoCi(2, 0) * alpha + R_AtoCi(2, 1) * beta + R_AtoCi(2, 2) + rho * p_AinCi(2, 0);
                    // Calculate jacobian
