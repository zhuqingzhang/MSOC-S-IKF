...
namespace ov_core {
    ...
    public:
        ...
        static void load_gt_file_tum(std::string path, std::map<double, Eigen::Matrix<double,8,1>>& gt_states) {
            ...
            while (std::getline(file, line)) {
                ...
                while (getline(s, field,' ')) {
                    ...
                    if(i==1||i==2||i==3)//txtytz
                    {
                        temp(i,0) = std::atof(field.c_str());
                    }
                    if(i==4||i==5||i==6) //qxqyqz
                    {
                        temp(i+1,0) = std::atof(field.c_str());
                    }
                    if(i==7) //qw
                    {
                        temp(i-3,0) = std::atof(field.c_str());
                    }
                    //temp: timestamp, tx ty tz qw qx qy qz
                    i++;
                }
                // Append to our groundtruth map
        ...
        static bool get_gt_state_tum(double timestep, Eigen::Matrix<double,8,1> &imustate, std::map<double, Eigen::Matrix<double,8,1>>& gt_states) {
            ...
            // Get the GT state vector
            Eigen::Matrix<double, 8, 1> state = gt_states[timestep];
