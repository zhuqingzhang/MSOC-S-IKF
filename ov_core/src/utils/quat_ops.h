...
namespace ov_core {
    ...
    inline Eigen::Matrix<double,3,1> compute_error(Eigen::Matrix<double, 4, 1> q1, Eigen::Matrix<double, 4, 1> q2)
        {
            //compute dq--> q1=dq*q2
            Eigen::Matrix3d dR=quat_2_Rot(q1)*quat_2_Rot(q2).transpose();
            Eigen::Vector4d dq=rot_2_quat(dR);
            Eigen::Vector3d dtheta(2*dq(0),2*dq(1),2*dq(2));
            return dtheta;
        }
    ...
    //expSE_3 
    //[dR, dp] * [R p] = [dR*R dR*p+dp]
    //[ 0,  1]   [0 1]   [  0      1  ]
    //param: delta_R and deltea_p in the vector space
    //return dp in the SE_3
    ...
    Eigen::Matrix3d Omega_1 = skew_x(xi_omega);
    ...
    inline Eigen::MatrixXd exp_sen3(Eigen::VectorXd v)
    {
        assert(v.rows()%3 == 0);
        int size_big = (v.rows()-3)/3 + 3;
        Eigen::MatrixXd mat = Eigen::MatrixXd::Identity(size_big,size_big);
        Eigen::MatrixXd sen3 = Eigen::MatrixXd::Zero(size_big,size_big);
        ...
        mat.block(0,0,3,3) = Eigen::Matrix3d::Identity() + A*wskew + B*wskew*wskew;
        // Matrices we need V and Identity
        Eigen::Matrix3d I_33 = Eigen::Matrix3d::Identity();
        Eigen::Matrix3d V = I_33 + B*wskew + C*wskew*wskew;

        for(int i=0; i<size_big-3; i++)
        {
            mat.block(0,3+i,3,1) = V * v.block(3+3*i,0,3,1);
        }

        return mat;
    }

    inline Eigen::MatrixXd exp_senm3(Eigen::VectorXd v1, Eigen::VectorXd v2)
    {
        assert((v1.rows()%3 == 0) && (v2.rows()%3 == 0));
        int size_all = (v1.rows()-3)/3 + 3 + v2.rows();
        Eigen::MatrixXd mat_big = Eigen::MatrixXd::Zero(size_all,size_all);
        Eigen::MatrixXd mat = exp_sen3(v1);
        mat_big.block(0,0,mat.rows(),mat.cols()) = mat;
        int id = mat.rows();
        for(int i=0; i<(v2.rows()/3); i++)
        {
            mat_big.block(id,id,3,3) = exp_so3(v2.block(3*i,0,3,1));
            id += 3;
        }
        assert(size_all == (mat.rows() + v2.rows()));
        assert(size_all == id);
        return mat_big;
    }
    ...
    inline Eigen::Matrix<double, 3, 3> Jl_so3_inv(Eigen::Matrix<double, 3, 1> w) {
        double theta = w.norm();
        if (theta < 1e-12) {
            return Eigen::MatrixXd::Identity(3, 3);
        } else {
            Eigen::Matrix<double, 3, 1> a = w / theta;
            Eigen::Matrix<double, 3, 3> J = theta/2.0/tan(theta/2)*Eigen::MatrixXd::Identity(3,3)+
                                            (1-theta/2.0/tan(theta/2))*a * a.transpose()-
                                            theta/2 * skew_x(a);
