...
Simulator::Simulator(VioManagerOptions& params_) {
    ...
    string sim_points= "/tmp/sim_points";
    of_point.open(sim_points.data());
    assert(of_point.is_open());
    string sim_gyro_bias="/tmp/sim_gbias_msckf_true.txt";
    of_gyro_bias.open(sim_gyro_bias.data());
    assert(of_gyro_bias.is_open());
    string sim_acc_bias="/tmp/sim_abias_msckf_true.txt";
                  ...
                  vector<vector<Eigen::VectorXd>> &res_pts3d_kf)
{
    ...
    for(; pt_num<1000; ) {
        ...
        // Generate a random depth
        // TODO: we should probably have this as a simulation parameter
        std::uniform_real_distribution<double> gen_depth(0.5,20);
        double depth = gen_depth(gen_state_init);
        ...
        cv::Mat mat1(1, 2, CV_32F);
        mat1.at<float>(0, 0) = u_dist;
        mat1.at<float>(0, 1) = v_dist;
        mat1 = mat1.reshape(2); // Nx1, 2-channel

        // Undistort this point to our normalized coordinates (false=radtan, true=fisheye)
        if(params.camera_fisheye.at(camid)) {
            cv::fisheye::undistortPoints(mat1, mat1, camK, camD);
        } else {
            cv::undistortPoints(mat1, mat1, camK, camD);
        }

        // Construct our return vector
        Eigen::Vector2f uv_norm_turb;
        mat1 = mat1.reshape(1); // Nx2, 1-channel
        uv_norm_turb(0) = mat1.at<float>(0, 0);
        uv_norm_turb(1) = mat1.at<float>(0, 1);
        ...
        {
            cout<<"p_FinCur(2) is out of range"<<endl;
            cout<<p_FinCur(2)<<endl;
            try_num_project++;
            if(try_num_project>1000)
            {
              break;
            }
            continue;
        ...
        vector<Vector2f> kfuvnorm_match;
       ...
       for(int i=1;i<kfs_ts_with_gt.size();i++)
       {
           ...
           if(uv_dist_kf(0) < 0 || uv_dist_kf(0) > params.camera_wh.at(camid).first || uv_dist_kf(1) < 0 || uv_dist_kf(1) > params.camera_wh.at(camid).second) {
            ...
            cv::Mat mat2(1, 2, CV_32F);
            mat2.at<float>(0, 0) = uv_dist_kf(0);
            mat2.at<float>(0, 1) = uv_dist_kf(1);
            mat2 = mat2.reshape(2); // Nx1, 2-channel

            // Undistort this point to our normalized coordinates (false=radtan, true=fisheye)
            if(params.camera_fisheye.at(camid)) {
                cv::fisheye::undistortPoints(mat2, mat2, camK, camD);
            } else {
                cv::undistortPoints(mat2, mat2, camK, camD);
            }

            // Construct our return vector
            Eigen::Vector2f uv_norm_kf_turb;
            mat2 = mat2.reshape(1); // Nx2, 1-channel
            uv_norm_kf_turb(0) = mat2.at<float>(0, 0);
            uv_norm_kf_turb(1) = mat2.at<float>(0, 1);

           kfindex_match.push_back(i);
           kfuv_match.push_back(uv_dist_kf);
           kfuvnorm_match.push_back(uv_norm_kf_turb);
      ...
      //      continue;
      //  }
      if(kfindex_match.size()<2)  //this 3dpt doesn't have enough observation for triangulate.
       {
           cout<<"match num is less than 2"<<endl;
           try_num++;
           if(try_num==500)
              break;
           continue;
    ...
    assert(pt_num==map_ptid_kfuvnorm.size());
    ...
    for(int pt_index=0;pt_index<pt_num;pt_index++)
    {
        vector<int> kfindces=map_ptid_kfindex[pt_index];
        vector<Vector2f> kfuvs=map_ptid_kfuv[pt_index];
        vector<Vector2f> kfuvnorms=map_ptid_kfuvnorm[pt_index];
        Vector3d p_FinG=map_ptid_3dgt[pt_index];
        Vector2f cur_uv=map_ptid_curuv[pt_index];
        Eigen::Vector3d pt3d_turb;
        Eigen::Vector3d pt3d_anchor_turb;
        bool s = single_triangulation(kfindces, kfuvs, kfuvnorms,kfs_pose_R_turb,kfs_pose_t_turb,pt3d_turb,pt3d_anchor_turb);
        
        if(!s)
          continue;
        else
          cout<<"single triangle success!"<<endl;
        
        s = single_gaussnewton(kfindces, kfuvs, kfuvnorms,kfs_pose_R_turb,kfs_pose_t_turb,pt3d_turb,pt3d_anchor_turb);
        if(!s)
          continue;
        else
          cout<<"single gaussnewton success!"<<endl;
        cout<<"p_FinG: "<<p_FinG.transpose()<<" p_FinG_turb: "<<pt3d_turb.transpose()<<endl;
        s=true;

        vector<int> kfindces_success;
        vector<Vector2f> kfuvs_success;
        for(int i=0;i<kfindces.size();i++)
        {
            ...
            cout<<"p_FinKF: "<<p_FinKF.transpose()<<" p_acnhor: "<<pt3d_anchor_turb.transpose()<<endl;
            if(p_FinKF(2) > 50 || p_FinKF(2) < 0.25)
            {
                s=false;
                continue;
            }
            ...
            if(abs(uv_dist_kf(0)-uv_dist_kf_reproject(0))>6||abs(uv_dist_kf(1)-uv_dist_kf_reproject(1))>6)
            {
              s=false;
              continue;
            }
            kfindces_success.push_back(kfindces[i]);
            kfuvs_success.push_back(uv_dist_kf);
        ...
        map_ptid_kfindex_success.insert({pt_index,kfindces_success});
        map_ptid_kfuv_success.insert({pt_index,kfuvs_success});
...
bool Simulator::single_triangulation(vector<int>& kfindeces,vector<Vector2f>& kfuvs,vector<Vector2f>& kfuvnorms,
        vector<Eigen::Matrix3d>& kfs_pose_R_turb,vector<Eigen::Vector3d>& kfs_pose_t_turb, Vector3d& pt_turb, Vector3d& pt_anchor)
{
  // Total number of measurements
    ...
    int anchor_id = kfindeces[0];
    // cout<<"feat anchor_img_id: "<<to_string(feat->anchor_img_id)<<endl;
    
    Eigen::Matrix<double,3,3> R_GtoA = kfs_pose_R_turb[anchor_id].transpose();
    Eigen::Matrix<double,3,1> p_AinG = kfs_pose_t_turb[anchor_id];
    // cout<<"anchor pose: "<<anchor_pose->quat().transpose()<<" "<<anchor_pose->pos().transpose()<<endl; 

    // Loop through each image for this feature
    for(int i=0; i<kfindeces.size();i++)
    {
        // Get the position of this image in the global
        Eigen::Matrix<double, 3, 3> R_GtoCi = kfs_pose_R_turb[kfindeces[i]].transpose();
        Eigen::Matrix<double, 3, 1> p_CiinG = kfs_pose_t_turb[kfindeces[i]];
        ...
        b_i << kfuvnorms[i](0), kfuvnorms[i](1), 1;
    ...
    pt_anchor = p_f;
    pt_turb = R_GtoA.transpose()*p_f + p_AinG;
...
bool Simulator::single_gaussnewton(vector<int>& kfindeces,vector<Vector2f>& kfuvs,vector<Vector2f>& kfuvnorms,
        vector<Eigen::Matrix3d>& kfs_pose_R_turb,vector<Eigen::Vector3d>& kfs_pose_t_turb,Vector3d& pt_turb, Vector3d& pt_anchor)
{
    //Get into inverse depth
    double rho = 1/pt_anchor(2);
    double alpha = pt_anchor(0)/pt_anchor(2);
    double beta = pt_anchor(1)/pt_anchor(2);
    ...
    double cost_old = compute_error(kfindeces,kfuvs,kfuvnorms,
        kfs_pose_R_turb,kfs_pose_t_turb,alpha,beta,rho);
    ...
    while (runs < 20 && lam <  1e10 && eps > 1e-6) {
        ...
        if (recompute) {
            ...
            for(int i=0;i<kfindeces.size();i++)
            {
                ...
                double hi1 = R_AtoCi(0, 0) * alpha + R_AtoCi(0, 1) * beta + R_AtoCi(0, 2) + rho * p_AinCi(0, 0);
                    ...
                    uv_norm<<kfuvnorms[i](0),kfuvnorms[i](1);
        ...
        double cost = compute_error(kfindeces,kfuvs,kfuvnorms,
        kfs_pose_R_turb,kfs_pose_t_turb,alpha,beta,rho);
    ...
    pt_anchor(0) = alpha/rho;
    pt_anchor(1) = beta/rho;
    pt_anchor(2) = 1/rho;

    // Get tangent plane to x_hat
    Eigen::HouseholderQR<Eigen::MatrixXd> qr(pt_anchor);
    ...
    if(pt_anchor(2) < 0.25
        || pt_anchor(2) > 40
        || (pt_anchor.norm() / base_line_max) > 40
        || std::isnan(pt_anchor.norm())) {
        return false;
    }

    // Finally get position in global frame
    pt_turb = R_GtoA.transpose()*pt_anchor+ p_AinG;
    return true;
}

double Simulator::compute_error(vector<int>& kfindeces,vector<Vector2f>& kfuvs,vector<Vector2f>& kfuvnorms,
