...
void VioManager::do_feature_propagate_update(double timestamp, Keyframe* cur_kf_0, Keyframe* cur_kf_1) {
   ...
   cout<<"finish slam delay update"<<endl;
    ...
    if(params.use_prior_map &&map_initialized&& do_map_kf_update)
    {
        ...
        for(int i=0;i<state->delay_loop_features.size();i++)
        {
            for(int j=0;j<state->delay_loop_features[i].size();j++)
            {
                ...
                // cout<<state->delay_loop_features[i][j]->timestamps[0][0]<<endl;
                // cout<<"feat id: "<<state->delay_loop_features[i][j]->featid<<endl;
            }
        }
        // cout<<"cur time: "<<state->_timestamp<<endl;
        // sleep(5);
        bool flag=false;
        state->iter_count=0;
        ...
        if(flag==false&&state->iter&&state->_options.iter_num==0)
        {
            ...
            if(loop_kf->PnPRANSAC(uv_norm,uv_3d_loop,p_loopIncur,R_loopTocur,params.match_num_thred))
            {
                size_t trans_id= state->transform_vio_to_map->id();
               ...
               if(state->_options.trans_fej)
               {
                ...
                {
                   ComputeRelativePose(cur_kf_0,loop_kf,p_loopIncur,R_loopTocur);
                //    // flag=updaterMSCKF->update_skf_with_KF6(state,kf_list,id,false,R_kf_cur,p_kf_cur,2);
                //    // flag=updaterMSCKF->update_skf_with_KF6(state,loop_kfs,index_kf,false,R_kf_cur,p_kf_cur,2);
                   count_1++;
                }
                // else if(trans<cur)
                // {
        ...
        // state->iter_count=1;
        //end recompute transform

        //iterative EKF
        if(state->iter&&state->_options.iter_num>0)
        {
            relinear_t1=boost::posix_time::microsec_clock::local_time();
            for(int iter=0;iter<state->_options.iter_num-1;iter++)
            {
                cout<<"in viomanager iter, iter is: "<<iter<<endl;
                cout<<"state->iter_count: "<<state->iter_count<<endl;
                bool flag=false;
                flag=updaterMSCKF->update_skf_with_KF3(state,feats_with_loop,true);
                if(flag==false)
                {
                    printf(RED "there is no useful information for update" RESET);
                    //reset linear value to estimate value;
                    for(auto var:state->_variables)
                    {
                        var->set_linp(var->value());
                    }
                    state->iter_count=state->_options.iter_num;
                    break;
                }
                
                if(state->iter_count==state->_options.iter_num)
                    break;
            }
            assert(state->iter_count==state->_options.iter_num);
            state->iter=false;
            state->iter_count=0;
            reset_map_count++;
            duration=state->_timestamp-last_kf_match_time;
    ...
    // for(Feature* feat: feats_with_loop){
    //     // good_features_MSCKF.push_back(feat->p_FinG);
    //     feat->to_delete = true;
    // }
...
void VioManager::loadPoseGraph2(std::string map_save_path, std::string pose_graph_filename, std::string keyframe_pose_filename)
{
    ...
    while (getline(f1,str))
    {
        ...
        //    cout<<"==="<<result<<endl;
           kf_ts=stod(result);
           kf_ts=floor(kf_ts*100)/100.0;  //保留2位小数
        //    cout<<"***kf_ts"<<to_string(kf_ts)<<endl;
           line>>result;
           match_num=stoi(result);
        ...
        //    cout<<"query_ts: "<<to_string(kf->loop_img_timestamp)<<endl;
           ...
           {
                // cout<<"kf->timestamp: "<<to_string(kf->time_stamp)<<endl;
                while(getline(f2,str1))  //timestamp,tx,ty,tz,qx,qy,qz,qw
                {
                    stringstream line1(str1);
                    line1>>res1;
            ...
            else if(line_num%2==0)//{q2d.x,q2d.y,kf2d.x,kf2d.y,kf3d.x,kf3d.y,kf3d.z} ...  
            {
                ...
                else
                {
                        while(getline(f2,str1))  //timestamp,tx,ty,tz,qx,qy,qz,qw
                        {
                            ...
                            double ts=stod(res1);
                            double timestp=ts;
                            ts=floor(ts*10000)/10000.0; //保留2位小数for kaist,eight,circle. 4 for euroc
                            /**for YQ**/
                            // stringstream line1(str1);
                            // line1>>res1;
                            // double ts=stod(res1);
...
//void VioManager::addKeyFrameIntoVoc(Keyframe *keyframe) {
...
bool VioManager::ComputeRelativePose2(Keyframe *cur_kf, Keyframe *loop_kf,Vector3d &p_loopIncur, Matrix3d &R_loopTocur) {
    ...
    if(state->_options.trans_fej)
    {
        R_vio_to_map= state->transform_vio_to_map->Rot_linp();
        p_vio_in_map= state->transform_vio_to_map->pos_linp();
    }
    else
    {
        R_vio_to_map= state->transform_vio_to_map->Rot_linp();
    ...
    // if(state->_options.trans_fej)
    // {
    //     R_vio_to_map=state->transform_vio_to_map->Rot_fej();
    //     p_vio_in_map=state->transform_vio_to_map->pos_fej();
    // }
    ...
    // state->_clones_IMU.at(state->_timestamp)->set_value(pose_cur);
...
vector<Keyframe*> VioManager::get_loop_kfs(State *state)
{
    ...
    ts=floor(ts*100)/100.0;  //保留4位小数 for euroc ;保留2位小数for kaist and eight circle
...
void VioManager::init_relative_transformation(State* state)
{
  ...
  Eigen::Vector3d trans_vins_in_map = Eigen::Vector3d::Zero();
  trans_vins_in_map(0) = trans_vins_in_map(0) + ax;
  trans_vins_in_map(1) = trans_vins_in_map(1) + ay;
  trans_vins_in_map(2) = trans_vins_in_map(2) + az;
  Quaterniond q_vins_to_map(1,0,0,0);
  Quaterniond dq(1,rx/2.0,ry/2.0,rz/2.0);
  Quaterniond q = q_vins_to_map*dq;
  q=q.normalized();
  ...
  Matrix<double,7,1> pose_vins_to_map = Matrix<double,7,1>::Zero();
  pose_vins_to_map<<q.x(),q.y(),q.z(),q.w(),trans_vins_in_map;
  // pose_vins_to_map<<0,0,0,1,0,0,0;
  
  cout<<"relative transform is initlizied: "<<pose_vins_to_map.transpose()<<endl;
  cout<<"error_rot: "<<rx<<" "<<ry<<" "<<rz<<endl;
  cout<<"error_p: "<<pose_vins_to_map.block(4,0,3,1).transpose()<<endl;
  ...
  pose_tranform->set_value(pose_vins_to_map);
  pose_tranform->set_linp(pose_vins_to_map);
  pose_tranform->set_fej(pose_vins_to_map);
