...
RosVisualizer::RosVisualizer(ros::NodeHandle &nh, VioManager* app, Simulator *sim) : _nh(nh), _app(app), _sim(sim) {
    ...
    pub_poseimu_rect = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/poseimu_rect", 2);
    ROS_INFO("Publishing: %s", pub_poseimu_rect.getTopic().c_str());
    pub_transform = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/transform", 2);
    ROS_INFO("Publishing: %s", pub_poseimu_rect.getTopic().c_str());
    pub_a_bias = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/a_bias",2);
    pub_g_bias = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/g_bias",2);
    ...
    pub_pathimu_rect = nh.advertise<nav_msgs::Path>("/ov_msckf/pathimu_rect", 2);
    ...
    pub_points_sim_matching_map = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/points_sim_matching_map", 2);
    ROS_INFO("Publishing: %s", pub_points_sim_matching_map.getTopic().c_str());
    pub_match_location = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/match_location", 3000);
    ...
    pub_match_image = nh.advertise<sensor_msgs::Image>("/ov_msckf/match_image", 2);
    ...
    if(save_total_state) {
        ...
        if(boost::filesystem::exists(filepath_matching_postion))
            boost::filesystem::remove(filepath_matching_postion);
...
void RosVisualizer::visualize() {
    ...
    cout<<"before publish images"<<endl;
    publish_images();
    cout<<"before publish state"<<endl;
    ...
    // publish state
    publish_state();
    cout<<"before publish feature"<<endl;
    // publish points
    publish_features();
    cout<<"before publish gt"<<endl;
    // Publish gt if we have it
    publish_groundtruth();
    cout<<"before publish kf"<<endl;
...
void RosVisualizer::publish_state() {
    ...
    Matrix3d R_MaptoVio;
    Vector3d t_MapinVio;
    Eigen::Matrix<double,6,6> cov_transform_ViotoMap;
    Eigen::Matrix<double,6,6> cov_transform_MaptoVio;
    if(state->set_transform)
    {
        R_ViotoMap= state->transform_vio_to_map->Rot();
        t_VioinMap= state->transform_vio_to_map->pos();
        R_MaptoVio = R_ViotoMap.transpose();
        t_MapinVio = - R_MaptoVio * t_VioinMap; 
        size_t id=state->transform_vio_to_map->id();
        cov_transform_ViotoMap = state->_Cov.block(id,id,6,6);
        Matrix<double,6,6> Jacobian = Matrix<double,6,6>::Zero();
        Jacobian.block(0,0,3,3) = - R_MaptoVio;
        Jacobian.block(3,0,3,3) = R_MaptoVio * skew_x(t_VioinMap);
        Jacobian.block(3,3,3,3) = - R_MaptoVio;
        cov_transform_MaptoVio = Jacobian * cov_transform_ViotoMap *Jacobian.transpose();
        
    }
    else
    {
        R_ViotoMap=Matrix3d::Identity();
        t_VioinMap=Vector3d(0,0,0);
        R_MaptoVio = R_ViotoMap.transpose();
        t_MapinVio = - R_MaptoVio * t_VioinMap; 
        
        cov_transform_ViotoMap=Matrix<double,6,6>::Identity()*state->_options.transform_cov;
        Matrix<double,6,6> Jacobian = Matrix<double,6,6>::Zero();
        Jacobian.block(0,0,3,3) = - R_MaptoVio;
        Jacobian.block(3,0,3,3) = R_MaptoVio * skew_x(t_VioinMap);
        Jacobian.block(3,3,3,3) = - R_MaptoVio;
    ...
    Quaterniond q_ViotoMap(R_ViotoMap);
    ...
    Eigen::Matrix<double,6,6> cov_transform_=Matrix<double,6,6>::Zero(); //in the form of pos,ori
    cov_transform_.block(0,0,3,3)=cov_transform_MaptoVio.block(3,3,3,3);
    cov_transform_.block(3,3,3,3)=cov_transform_MaptoVio.block(0,0,3,3);
    cov_transform_.block(0,3,3,3)=cov_transform_MaptoVio.block(3,0,3,3);
    cov_transform_.block(3,0,3,3)=cov_transform_MaptoVio.block(0,3,3,3);
    for(int r=0; r<6; r++) {
        for(int c=0; c<6; c++) {
            transform.pose.covariance[6*r+c] = cov_transform_(r,c);
        }
    }
    if(state->set_transform)
      pub_transform.publish(transform);  
    cout<<"after publish transform"<<endl;

    // geometry_msgs::PoseWithCovarianceStamped transform;
    // transform.header=poseIinM.header;
    // transform.pose.pose.orientation.x=q_ViotoMap.x();
    // transform.pose.pose.orientation.y=q_ViotoMap.y();
    // transform.pose.pose.orientation.z=q_ViotoMap.z();
    // transform.pose.pose.orientation.w=q_ViotoMap.w();
    // transform.pose.pose.position.x=t_VioinMap.x();
    // transform.pose.pose.position.y=t_VioinMap.y();
    // transform.pose.pose.position.z=t_VioinMap.z();
    // Eigen::Matrix<double,6,6> cov_transform_=Matrix<double,6,6>::Zero(); //in the form of pos,ori
    // cov_transform_.block(0,0,3,3)=cov_transform.block(3,3,3,3);
    // cov_transform_.block(3,3,3,3)=cov_transform.block(0,0,3,3);
    // cov_transform_.block(0,3,3,3)=cov_transform.block(3,0,3,3);
    // cov_transform_.block(3,0,3,3)=cov_transform.block(0,3,3,3);
    // for(int r=0; r<6; r++) {
    //     for(int c=0; c<6; c++) {
    //         transform.pose.covariance[6*r+c] = cov_transform_(r,c);
    //     }
    ...
    cout<<"after publish poseimu"<<endl;
    // cout<<"timestamp: "<<to_string(timestamp_inI)<<endl;
    // cout<<"covariance: "<<endl<<covariance_posori<<endl;
    ...
    // vars.push_back(state->transform_vio_to_map);
    ...
    // R_VinstoMap=state->transform_vio_to_map->Rot();
    // p_VinsinMap=state->transform_vio_to_map->pos();
    ...
    Eigen::Matrix<double,6,6> cov_rect_=Eigen::Matrix<double,6,6>::Identity();
    for(int r=0; r<6; r++) {
        for(int c=0; c<6; c++) {
    ...
    pub_poseimu_rect.publish(posewithCov_rect);
     ...
     cout<<"after publish poserect"<<endl;
    // else
    // {
    ...
    cout<<"after publish bias_a"<<endl;
    ...
    pub_g_bias.publish(g_bias);  
     cout<<"after publish bias_g"<<endl;
